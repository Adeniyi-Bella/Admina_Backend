...
//       const result = await service.deleteAllDocuments(user);
...
//       await service.deleteAllDocuments(user);

//       expect(logger.info).toHaveBeenCalledWith('No documents found for deletion', { user });
//     });

//     it('should throw error if deletion fails', async () => {
//       (Document.deleteMany as jest.Mock).mockReturnValue({ exec: () => Promise.reject(new Error('DB error')) });

//       await expect(service.deleteAllDocuments(user)).rejects.toThrow('Failed to delete all documents: DB error');
...
//       await expect(service.getAllDocumentsByUserId('', 10, 0)).rejects.toThrow('Valid user is required');
//     });
...
//       const result = await service.getAllDocumentsByUserId(user, 10, 0);
...
//       const result = await service.createDocumentByUserId({ user, docId });
...
//       await expect(service.createDocumentByUserId({ user, docId })).rejects.toThrow(
...
//     it('should throw if user or docId is missing', async () => {
//       await expect(service.getDocument('', docId)).rejects.toThrow('Valid user and docId are required');
//     });
...
//       const result = await service.getDocument(user, docId);
//       expect(result).toEqual({ docId });
//     });
...
//       await expect(service.getDocument(user, docId)).rejects.toThrow('Document not found');
//     });
//   });

//   describe('deleteDocument', () => {
//     it('should throw if user or docId is missing', async () => {
//       await expect(service.deleteDocument('', docId)).rejects.toThrow('Valid user and docId are required');
//     });

//     it('should delete document successfully', async () => {
//       (Document.deleteOne as jest.Mock).mockReturnValue({ exec: () => Promise.resolve({ deletedCount: 1 }) });

//       const result = await service.deleteDocument(user, docId);
//       expect(result).toBe(true);
//     });
...
//     it('should throw if user/docId missing', async () => {
//       await expect(service.updateDocument('', docId, {})).rejects.toThrow('Valid user and docId are required');
//     });

//     it('should throw if updates missing', async () => {
//       await expect(service.updateDocument(user, docId, {})).rejects.toThrow('Valid update data is required');
//     });
...
//       const result = await service.updateDocument(user, docId, { title: 'new' });
//       expect(result).toEqual({ docId });
//     });
...
//         'Valid user and docId are required'
//       );
//     });
...
//       const result = await service.updateActionPlan(user, docId, 'create', { title: 'Test' });
//       expect(result).toEqual({ docId });
//     });
...
//       const result = await service.updateActionPlan(user, docId, 'delete', undefined, 'ap1');
//       expect(result).toEqual({ docId });
//     });
...
//       const result = await service.updateActionPlan(user, docId, 'update', { title: 'new' }, 'ap1');
//       expect(result).toEqual({ docId });
//     });

//     it('should throw on invalid action', async () => {
//       await expect(service.updateActionPlan(user, docId, 'invalid' as any)).rejects.toThrow(
